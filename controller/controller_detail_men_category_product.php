...
//so ban ghi tren 1 trang
	$record_perpage =6;
	//tong so ban ghi
	$total_record = fetch_one("select count(*) as total from tbl_product where c_sex='Nam'");
	//lay tong so trang
	...
	$arr_detail_men_category_product=fetch("select * from tbl_product where c_sex='Nam' order by pk_product_id desc limit $from,$record_perpage");
	//$arr_detail_men_category_product=fetch("select * from tbl_product where c_sex='Nam'");
	$arr_category_product=fetch("select * from tbl_category_product where c_parent_id=0");
	// tạo ra sản phẩm nội bật bên  tay phải trang
	$arr_sanphamnoibat=fetch("select *  from tbl_product where c_sanphamnoibat=1 order by pk_product_id desc limit 0,4");
	include_once("view/view_detail_men_category_product.php");
?>